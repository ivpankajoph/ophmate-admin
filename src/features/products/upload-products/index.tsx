...
const safeResult = (raw: any): UploadResult => {
  ...
  const errors = Array.isArray(raw.errors)
    ? raw.errors.map((e: any) => ({
        ...
        productName: typeof e.productName === 'string' ? e.productName : 'Unknown',
        errors: Array.isArray(e.errors) && e.errors.length > 0 
          ? e.errors.filter((err: any) => typeof err === 'string')
    ...
    try {
      ...
      if (response.ok) {
        ...
        setResult({
          ...
          successCount: 0,
          failureCount: 0,
          errors: [{ row: 0, productName: 'System', errors: [message] }],
          successfulProducts: [],
        });
      }
    } catch (error: any) {
      ...
      setResult({
        ...
        successCount: 0,
        failureCount: 0,
        errors: [{ row: 0, productName: 'System', errors: [errorMsg] }],
        successfulProducts: [],
      });
    } finally {
      setUploading(false);
