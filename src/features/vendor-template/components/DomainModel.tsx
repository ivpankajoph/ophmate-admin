...
export function DomainModal({ open, setOpen }: Type) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setResponse(e?.response?.data || e?.message || 'An unknown error occurred')
    } finally {
      setLoading(false)
    }
  }
  ...
  const renderResponse = (data: any) => {
    if (typeof data === 'string') {
      return <p className="whitespace-pre-wrap text-sm">{data}</p>
    }

    if (typeof data === 'object' && data !== null) {
      try {
        return (
          <pre className="text-xs p-3 bg-gray-50 rounded-md overflow-auto max-h-60 border border-gray-200">
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        className="max-w-lg p-6 space-y-4 rounded-2xl border shadow-lg"
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold">Connect Your Domain</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          {response !== null && (
            <div className="mt-4">
              ...
              <Button className='mt-1'>Verify Domain </Button>
            </div>
          )}
        </div>
      </DialogContent>
